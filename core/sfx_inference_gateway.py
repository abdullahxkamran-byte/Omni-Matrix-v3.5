...
class SFX_Inference_Gateway:
    ...
    def _load_model(self):
        ...
        try:
            ...
            print(f"[SFX_Gateway] Loading Neural Diffuser ({self.model_id}) to {self.device}...", flush=True)
    ...
    def _unload_model(self):
        if self.pipe is not None:
            ...
            print("[SFX_Gateway] VRAM Cleared for downstream agents.", flush=True)

    def generate_neural_sfx(self, prompt: str, duration_sec: float, guidance_scale: float = 3.5, seed: int = 42) -> np.ndarray:
        self._load_model()
        generator = torch.Generator(device=self.device).manual_seed(seed)
        
        print(f"[SFX_Gateway] Synthesizing Neural Body: '{prompt}'", flush=True)
